...
ParticleBase::ParticleBase(sf::RenderWindow& window, float radius)
    :Particle(sf::Vector2f(rand() % window.getSize().x, window.getSize().y - radius * 5), radius)
{
    direction = sf::Vector2f(0.f, -(rand() % 4 + 8));
}

void ParticleBase::update()
{
    acceleration *= 0.f;
    acceleration += sf::Vector2f(0.f, 0.1f);
